...
function prepareImport(value, filename) {
  ...
  if (!records.length) throw new Error('No usable project records were found in this file.');
  return { records, format: adapted.format, report: imported.report, notes: adapted.notes };
}

function adaptSiteIndex(value) {
  const detected = detectOlcCatalog(value);
  if (detected !== 'unknown') return { value, format: detected, notes: [] };
  const candidates = firstArray(value, ['pages', 'entries', 'items', 'documents', 'urls', 'index']);
  if (!candidates) throw new Error('This JSON file does not contain a recognized CuratorOS catalog, OLC manifest, or site-index page array.');
  const notes = [];
...
function pageEntryToRecord(entry, index, notes) {
  if (typeof entry === 'string') return { id: entry, type: inferPageType(entry), title: titleFromPath(entry), status: 'published', url: entry };
  ...
  if (!title) { notes.push(`Entry ${index + 1} was skipped because it has no title, name, or usable path.`); return null; }
  const type = entry.type || entry.kind || entry.recordType || inferPageType(url, entry);
  return { ...entry, id: entry.id || entry.slug || url || title, type, title, status: entry.status || 'published', url, data: { ...(entry.data && typeof entry.data === 'object' ? entry.data : {}), pageUrl: url || undefined, section: entry.section || entry.category || undefined } };
}
...
function titleFromPath(path = '') {
  ...
  return decodeURIComponent(last).replace(/\.html?$/i, '').replace(/[-_]+/g, ' ').replace(/\b\w/g, (letter) => letter.toUpperCase());
}
...
function showImportReview(prepared, filename) {
  ...
  dialog.innerHTML = `<form method="dialog" class="project-import-card"><span class="eyebrow">Project import review</span><h3>${escapeHtml(filename)}</h3><p>CuratorOS recognized this as <strong>${escapeHtml(format)}</strong>. Nothing has been replaced yet.</p><div class="metrics">${metric(records.length, 'Records ready')}${metric(report.skipped?.length || 0, 'Skipped')}${metric(warningCount, 'Warnings')}${metric(report.errors?.length || 0, 'Errors')}</div><section class="panel"><h4>Records by type</h4><div class="badges">${Object.entries(typeCounts).sort().map(([type, count]) => `<span class="badge">${escapeHtml(type)}: ${count}</span>`).join('')}</div></section>${renderIssues('Warnings', [...(report.warnings || []).map(issueText), ...notes])}${renderIssues('Skipped entries', (report.skipped || []).map(issueText))}${renderIssues('Errors', (report.errors || []).map(issueText))}<p><strong>Importing will replace the permanent Project Records corpus.</strong> The browser cache is updated only after the permanent save succeeds.</p><div class="actions"><button type="button" data-cancel-import>Cancel</button><button type="button" data-confirm-import>Import ${records.length} records</button></div></form>`;
...
async function commitImport(prepared, filename) {
  try {
    const store = window.CuratorOSProjectRecordsStore;
    if (!store) throw new Error('Permanent Project Records store is not available in this build.');
    await store.replace(prepared.records, `import:${filename}`);
    localStorage.setItem(ORIGIN_KEY, JSON.stringify({ filename, format: prepared.format, recordCount: prepared.records.length, importedAt: new Date().toISOString(), report: prepared.report }));
    closeDialog();
    location.reload();
  } catch (error) {
    showImportError(`The permanent Project Records store could not be updated. ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function renderIssues(title, issues) { if (!issues.length) return ''; return `<details class="panel"><summary>${escapeHtml(title)} (${issues.length})</summary><ul>${issues.slice(0, 100).map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>${issues.length > 100 ? `<p>Only the first 100 are shown.</p>` : ''}</details>`; }
function issueText(issue) { if (typeof issue === 'string') return issue; return issue.message || issue.reason || JSON.stringify(issue); }
...
function closeDialog() { const dialog = document.querySelector('#project-import-review'); if (!dialog) return; try { dialog.close(); } catch {} dialog.remove(); }
