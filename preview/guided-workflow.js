...
const WORKFLOWS={
  ingest:{
    title:'Learn from an existing webpage',
    purpose:'Use an existing OceanLiners.net page to create or update structured knowledge.',
    steps:[
  ...
  create:{
    title:'Create a new webpage from CuratorOS knowledge',
    purpose:'Combine existing records into a new OceanLiners.net page.',
    steps:[
  ...
  update:{
    title:'Update an existing webpage from CuratorOS knowledge',
    purpose:'Use current structured knowledge to prepare a reviewed replacement for an existing public page.',
    steps:[
  ...
  review:{
    title:'Review or clean up a Project Record',
    purpose:'Inspect an existing record, its identity, relationships, and supporting evidence.',
    steps:[
  ...
  explore:{
    ...
    purpose:'Browse connections, discover patterns, and turn useful findings into reusable collections.',
    steps:[
...
function render(){
  if(!app)return;
  const flow=WORKFLOWS[state.workflow];
  app.innerHTML=flow?renderFlow(flow):renderStart();
  bind();
}

function renderStart(){
  return `<section class="panel guide-flow-hero"><span class="eyebrow">Start Here · Guided Workflow</span><h3>What are you trying to do?</h3><p>Choose the outcome you want. CuratorOS will show you the exact order of workspaces, what to click or select in each one, and what you should have before moving on.</p></section>
  <section class="guide-flow-grid">
    ${card('ingest','Learn from an existing webpage','Start with one OceanLiners.net page and turn it into reviewed Project Records.')}
    ${card('create','Create a new webpage','Start with knowledge already in CuratorOS and build a new page package.')}
    ${card('update','Update an existing webpage','Use reviewed records to prepare a replacement for a page that already exists.')}
    ${card('review','Review a Project Record','Clean up one record, its identity, relationships, and evidence.')}
    ${card('explore','Explore the knowledge base','Follow relationships, discover clusters, and save useful collections.')}
  </section>`;
}
...
function renderFlow(flow){
  ...
  const progress=Math.round(((step+1)/flow.steps.length)*100);
...
function bind(){
  document.querySelectorAll('[data-start-flow]').forEach(b=>b.addEventListener('click',()=>{state={workflow:b.dataset.startFlow,step:0,subject:''};save();render();}));
  document.querySelector('#change-workflow')?.addEventListener('click',()=>{state={workflow:'',step:0,subject:state.subject||''};save();render();});
  document.querySelector('#workflow-subject')?.addEventListener('input',e=>{state.subject=e.target.value;save();});
  document.querySelector('#workflow-next')?.addEventListener('click',()=>{const flow=WORKFLOWS[state.workflow];state.step=Math.min(state.step+1,flow.steps.length-1);save();render();});
  document.querySelector('#workflow-back')?.addEventListener('click',()=>{state.step=Math.max(0,state.step-1);save();render();});
  document.querySelector('#workflow-finish')?.addEventListener('click',()=>{state={workflow:'',step:0,subject:''};save();render();});
  document.querySelectorAll('[data-jump-step]').forEach(b=>b.addEventListener('click',()=>{state.step=Number(b.dataset.jumpStep)||0;save();render();}));
  document.querySelectorAll('[data-open-workspace]').forEach(b=>b.addEventListener('click',()=>openWorkspace(b.dataset.openWorkspace)));
}

function openWorkspace(id){
  const selector=id==='records'?'[data-view="records"]':`#${CSS.escape(id)}`;
  const target=document.querySelector(selector);
  if(!target)return alert('That CuratorOS workspace is not available in this build.');
  save();
  target.click();
  showGuideBar();
}

function showGuideBar(){
  document.querySelector('#guided-workflow-bar')?.remove();
  const flow=WORKFLOWS[state.workflow];if(!flow)return;
  const current=flow.steps[Math.min(state.step,flow.steps.length-1)];
  const bar=document.createElement('div');bar.id='guided-workflow-bar';bar.className='guided-workflow-bar';bar.innerHTML=`<div><small>Guided workflow · Step ${state.step+1} of ${flow.steps.length}</small><strong>${esc(current.title)}</strong>${state.subject?`<span>${esc(state.subject)}</span>`:''}</div><div><button type="button" data-return-guide>Return to guide</button><button type="button" data-complete-guide-step>Finished →</button></div>`;
  document.body.append(bar);
  bar.querySelector('[data-return-guide]')?.addEventListener('click',()=>{activate();render();});
  bar.querySelector('[data-complete-guide-step]')?.addEventListener('click',()=>{state.step=Math.min(state.step+1,flow.steps.length-1);save();activate();render();bar.remove();});
}
