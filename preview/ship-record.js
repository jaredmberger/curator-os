...
const SHIP_SECTIONS=[
  {title:'Core identity',fields:[
    ['prefix','Prefix'],['shippingLine','Shipping line / operator'],['builder','Built by'],['yardNumber','Yard number'],['buildLocation','Built at'],['registry','Registry / flag'],['shipClass','Ship class'],['sisterShips','Sister ships','list']
  ]},
  ...
  {title:'Dimensions & machinery',fields:[
    ['grossTonnage','Gross tonnage'],['netTonnage','Net tonnage'],['displacement','Displacement'],['length','Length'],['beam','Beam'],['draft','Draft'],['propulsion','Propulsion'],['engines','Engines'],['power','Power'],['serviceSpeed','Service speed'],['maximumSpeed','Maximum speed'],['funnels','Funnels'],['masts','Masts']
  ]},
  {title:'Capacity & service',fields:[
    ['passengerCapacity','Passenger capacity'],['crew','Crew'],['routes','Routes','list'],['wartimeService','Wartime service','multiline'],['majorRefits','Major refits','multiline'],['serviceNotes','Service notes','multiline']
  ]},
  {title:'End of service & fate',fields:[
    ['serviceEndDate','Service ended'],['fate','Fate'],['fateDate','Fate date'],['fateLocation','Fate location']
  ]}
];
...
function enhanceShipInspector(inspector,record){
  ...
  const card=inspector.querySelector('.record-inspector-card');
  if(!card)return;
  ...
  if(actions){
    const generic=[...actions.querySelectorAll('button')].find(b=>/edit permanent record/i.test(b.textContent||''));
    if(generic)generic.remove();
    const edit=document.createElement('button');edit.type='button';edit.textContent='Edit Ship Record';edit.className='ship-record-primary';
    edit.addEventListener('click',()=>openShipEditor(record.id));
...
function renderShipFacts(record){
  ...
  return `<section class="ship-record-overview"><div class="ship-record-title-row"><div><span class="eyebrow">The ship</span><h4>${esc(record.title||'Unnamed ship')}</h4></div><div class="ship-record-keyfacts">${quickFact('Shipping line',data.shippingLine)}${quickFact('Built by',data.builder)}${quickFact('Launched',data.launchDate)}${quickFact('Fate',data.fate)}</div></div>${SHIP_SECTIONS.map(section=>renderFactSection(section,data)).join('')}</section>`;
}

function renderFactSection(section,data){
  const values=section.fields.map(([key,labelText])=>[labelText,displayShipValue(data[key])]).filter(([,value])=>value);
  if(!values.length)return'';
  return `<section class="ship-record-section"><h4>${esc(section.title)}</h4><dl class="ship-record-facts">${values.map(([labelText,value])=>`<div><dt>${esc(labelText)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl></section>`;
}
...
function openShipEditor(recordId){
  ...
  const record=existing||{id:'',title:'',type:'ship',status:'review',summary:'',data:{},sources:[],relationships:[],notes:[],tags:[]};
  const data=shipData(record);
  document.querySelector('#ship-record-editor')?.remove();
  const dialog=document.createElement('dialog');dialog.id='ship-record-editor';
  dialog.innerHTML=`<form class="ship-record-editor-card"><header class="ship-record-editor-header"><div><span class="eyebrow">${existing?'Edit canonical ship record':'New canonical ship record'}</span><h3>${esc(existing?.title||'Create Ship Record')}</h3><p>This record represents the actual ship. Enter the concrete facts first; evidence, relationships, and notes remain attached underneath.</p></div><button type="button" data-close-ship aria-label="Close">×</button></header>
  ...
  <details class="ship-editor-advanced"><summary>Evidence, relationships & curatorial notes</summary><p>These support the concrete ship record; they do not replace it.</p>${textarea('Sources — JSON array','ship-sources',JSON.stringify(record.sources||[],null,2),7)}${textarea('Relationships — JSON array','ship-relationships',JSON.stringify(record.relationships||[],null,2),7)}${textarea('Curatorial notes — JSON array','ship-notes',JSON.stringify(record.notes||[],null,2),7)}</details>
  ...
  dialog.addEventListener('cancel',()=>closeShipEditor());
  dialog.showModal();
}
...
async function saveShipRecord(before,dialog,isEdit){
  ...
  try{
    ...
    const id=value(dialog,'#ship-id').trim()||makeShipId(title);
    const records=readShipRecords();
    if(!isEdit&&records.some(r=>r.id===id))throw new Error(`A Project Record with ID ${id} already exists.`);
    const oldData=before.data&&typeof before.data==='object'?before.data:{};
    const data={...oldData};
    ...
    const after={...before,id,title,type:'ship',status:value(dialog,'#ship-status')||'review',summary:value(dialog,'#ship-summary').trim(),data,sources:parseArray(value(dialog,'#ship-sources'),'Sources'),relationships:parseArray(value(dialog,'#ship-relationships'),'Relationships'),notes:parseArray(value(dialog,'#ship-notes'),'Curatorial notes'),metadata:{...(before.metadata||{}),shipSchemaVersion:1,[isEdit?'permanentlyEditedAt':'permanentlyCreatedAt']:now},origin:before.origin||{kind:'curatoros-native',createdAt:now}};
    const next=isEdit?records.map(r=>r.id===before.id?after:r):[...records,after];
    const store=window.CuratorOSProjectRecordsStore;if(!store)throw new Error('Permanent Project Records store is not available.');
...
function shipData(record){const d=record?.data&&typeof record.data==='object'?record.data:{};return {...d,shippingLine:d.shippingLine||d.line||d.operator||d.company||'',builder:d.builder||d.builtBy||d.shipbuilder||'',launchDate:d.launchDate||d.launched||d.launchYear||'',enteredServiceDate:d.enteredServiceDate||d.enteredService||d.serviceStart||'',grossTonnage:d.grossTonnage||d.tonnage||d.grt||'',serviceSpeed:d.serviceSpeed||d.speed||'',shipClass:d.shipClass||d.class||'',sisterShips:d.sisterShips||d.sisters||[]};}
...
function changedShipFields(a,b){const fields=['title','status','summary','data','sources','relationships','notes'];return fields.filter(k=>JSON.stringify(a?.[k]??null)!==JSON.stringify(b?.[k]??null));}
