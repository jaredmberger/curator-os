...
function openWorkspace(id){save();const ok=window.CuratorOSNavigate?.open?.(id);if(ok)return;window.dispatchEvent(new CustomEvent('curatoros:navigate-workspace',{detail:{workspace:id}}));setTimeout(()=>{const selector=id==='records'?'[data-view="records"]':`#${CSS.escape(id)}`;const target=document.querySelector(selector);if(target&&!target.classList.contains('active'))target.click();},0);}
