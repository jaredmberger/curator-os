...
function openWorkspace(id){const selector=id==='records'?'[data-view="records"]':`#${CSS.escape(id)}`;const target=document.querySelector(selector);if(!target)return alert('That CuratorOS workspace is not available in this build.');save();document.querySelector('#guided-workflow-bar')?.remove();target.click();}
