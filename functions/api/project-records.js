export async function onRequestGet(context){
  ...
  if(!store)return json({error:'CURATOROS_RECORDS binding is not configured.'},500);
...
export async function onRequestPut(context){
  ...
  return json({ok:true,version,recordCount:body.records.length,updatedAt:payload.updatedAt},200);
}

function json(value,status){return new Response(JSON.stringify(value,null,2),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}})}
