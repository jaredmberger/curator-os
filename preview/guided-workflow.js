...
function openWorkspace(id){
  save();
  const nav=window.CuratorOSNavigate;
  if(nav?.open?.(id))return;
  const selector=id==='records'?'[data-view="records"]':id==='help'?'[data-view="help"]':id==='findings'?'[data-view="findings"]':`#${CSS.escape(id)}`;
  const target=document.querySelector(selector);
  if(!target){alert('That CuratorOS workspace is not available in this build.');return;}
