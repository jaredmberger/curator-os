...
function enhanceInspector() {
  ...
  edit.type = 'button';
  edit.textContent = 'Edit local record';
...
function inferRecordId(dialog) {
  const identity = [...dialog.querySelectorAll('.record-detail-grid dt')]
    .find((node) => node.textContent.trim().toLowerCase() === 'record id');
...
function enhanceRecordsView() {
  ...
  section.innerHTML = `
    <div>
      <span class="eyebrow">Local editing workspace</span>
      <h4>${changes.length} pending change${changes.length === 1 ? '' : 's'}</h4>
      <p>Edits remain in this browser until you export or discard them. Website files and GitHub are untouched.</p>
    </div>
    <div class="pending-change-actions">
      <button type="button" data-review-changes${changes.length ? '' : ' disabled'}>Review changes</button>
      <button type="button" data-export-changes${changes.length ? '' : ' disabled'}>Export change set</button>
      <button type="button" data-discard-changes${changes.length ? '' : ' disabled'}>Discard all</button>
    </div>`;
  ...
  section.querySelector('[data-discard-changes]')?.addEventListener('click', discardAllChanges);
}
...
function openEditor(recordId) {
  ...
  dialog.innerHTML = `
    <form class="record-editor-card">
      <header class="record-editor-header">
        <div><span class="eyebrow">Local record editor</span><h3>${escapeHtml(record.title || record.id)}</h3></div>
        <button type="button" data-close-editor aria-label="Close editor">×</button>
      </header>
      <p class="record-editor-safety">This creates a local pending change. It does not alter the imported source file or GitHub.</p>
      <div class="record-editor-grid">
        ${field('Title', 'edit-title', record.title || '')}
        ${field('Type', 'edit-type', record.type || 'record')}
        ${selectField('Status', 'edit-status', record.status || 'review', ['draft','review','published','archived'])}
        ${field('Record ID', 'edit-id', record.id || '', true)}
      </div>
      ${textarea('Summary', 'edit-summary', record.summary || '', 4)}
      ${textarea('Tags — one per line', 'edit-tags', (record.tags || []).join('\n'), 4)}
      ${textarea('Structured data — JSON object', 'edit-data', JSON.stringify(record.data || {}, null, 2), 10, 'json')}
      ${textarea('Sources — JSON array', 'edit-sources', JSON.stringify(record.sources || [], null, 2), 8, 'json')}
      ${textarea('Relationships — JSON array', 'edit-relationships', JSON.stringify(record.relationships || [], null, 2), 8, 'json')}
      ${textarea('Curatorial notes — JSON array', 'edit-notes', JSON.stringify(record.notes || [], null, 2), 8, 'json')}
      <div class="record-editor-error" role="alert" hidden></div>
      <footer class="record-editor-actions">
        <button type="button" data-close-editor>Cancel</button>
        <button type="submit">Save local change</button>
      </footer>
    </form>`;
  ...
  dialog.querySelector('form')?.addEventListener('submit', (event) => {
    ...
    saveRecordEdit(record, dialog);
  });
  dialog.addEventListener('cancel', () => closeDialog('#project-record-editor'));
  dialog.showModal();
}

function saveRecordEdit(before, dialog) {
  const errorBox = dialog.querySelector('.record-editor-error');
  try {
    const after = {
      ...before,
      title: value(dialog, '#edit-title').trim(),
      type: value(dialog, '#edit-type').trim() || 'record',
    ...
    window.dispatchEvent(new CustomEvent('curatoros:records-changed'));
  } catch (error) {
    if (errorBox) {
...
function openChangeReview() {
  ...
  dialog.innerHTML = `
    <section class="change-review-card">
      <header class="record-editor-header">
        <div><span class="eyebrow">Pending local changes</span><h3>${changes.length} changed record${changes.length === 1 ? '' : 's'}</h3></div>
        <button type="button" data-close-change-review aria-label="Close">×</button>
      </header>
      <div class="change-review-list">
        ${changes.map((change) => `
          <article>
            <div><strong>${escapeHtml(change.title || change.recordId)}</strong><p>${escapeHtml(change.recordId)}</p></div>
            <p><strong>Changed:</strong> ${escapeHtml((change.fields || []).join(', ') || 'record content')}</p>
            <p><small>${escapeHtml(formatDate(change.changedAt))}</small></p>
            <div class="actions"><button type="button" data-revert-record="${escapeHtml(change.recordId)}">Revert this record</button></div>
          </article>`).join('')}
      </div>
      <footer class="record-editor-actions"><button type="button" data-close-change-review>Close</button><button type="button" data-export-review>Export change set</button></footer>
    </section>`;
  document.body.append(dialog);
  ...
  dialog.querySelectorAll('[data-revert-record]').forEach((button) => button.addEventListener('click', () => revertRecord(button.dataset.revertRecord)));
...
function revertRecord(recordId) {
  const changes = readChanges();
  const change = changes.find((item) => item.recordId === recordId);
  if (!change || !confirm(`Revert local edits to ${change.title || recordId}?`)) return;
  const records = readRecords().map((record) => record.id === recordId ? change.before : record);
  const remaining = changes.filter((item) => item.recordId !== recordId);
  localStorage.setItem(CATALOG_KEY, JSON.stringify(records));
  localStorage.setItem(CHANGE_KEY, JSON.stringify(remaining));
  if (!remaining.length) localStorage.removeItem(BASELINE_KEY);
  closeDialog('#project-change-review');
  window.dispatchEvent(new CustomEvent('curatoros:records-changed'));
}
...
function discardAllChanges() {
  const changes = readChanges();
  if (!changes.length || !confirm(`Discard all ${changes.length} pending local change${changes.length === 1 ? '' : 's'}?`)) return;
  const baseline = readJson(BASELINE_KEY, null);
  if (Array.isArray(baseline)) localStorage.setItem(CATALOG_KEY, JSON.stringify(baseline));
...
function exportChangeSet() {
  ...
  const payload = {
    format: 'curatoros-project-change-set',
    formatVersion: 1,
  ...
  anchor.href = url;
  anchor.download = `curatoros-change-set-${new Date().toISOString().slice(0,10)}.json`;
