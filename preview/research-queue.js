...
function rqOpenRecord(id){sessionStorage.setItem('curatoros.openRecordId',id);window.CuratorOSNavigate?.open?.('records');setTimeout(()=>{const button=[...document.querySelectorAll('[data-record-id]')].find(b=>b.dataset.recordId===id);if(button){button.click();return}const search=document.querySelector('#record-search');if(search){search.value=id;search.dispatchEvent(new Event('input',{bubbles:true}));setTimeout(()=>document.querySelector(`[data-record-id="${CSS.escape(id)}"]`)?.click(),80)}},120);}
